...
| `mode` | `"normal"` 或 `"advisor"`。Advisor 模式下主 Agent 默认用 `sub_model`（次模型）以降低 token 成本，仅在 plan mode 内临时切换为 `model`（主模型）深度推理。仅在 `sub_model` 非空且不等于 `model` 时生效 | `"normal"` |
...
`web_search` 工具默认使用 DuckDuckGo 搜索引擎，无需任何配置。如需更好的搜索结果质量，可通过环境变量切换到 Brave Search API：
...
配置优先级：环境变量 `BRAVE_API_KEY` > `settings.json` 的 `web_search.brave_api_key`。未配置时自动使用 DuckDuckGo。
