...
DeepSeek 的前缀缓存机制:每次请求时,API 从 `messages[0]` 开始逐条比对,找到与上次请求的最长公共前缀。命中部分按缓存价计费,未命中部分按标准价计费。**缓存命中与未命中的输入价格差异巨大**——以 V4-Flash 和 V4-Pro 为例,缓存命中价格仅为未命中的 **1/30**(2026-08 调价后;此前为 1/50 ~ 1/120)。
...
4. **单调边界保证**：压缩决策表（`compactionDecisionSet`）+ 三 cursor 机制确保每条消息只被压缩一次，不会反复修改导致缓存失效。
