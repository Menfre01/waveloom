...
### Added
- **Prefix-cache alignment for forked sub-agents**: the first fork request is now byte-identical to the parent's last cached payload (messages cut exactly at the parent's previous request load, tools array matching the parent), so DeepSeek prefix-cache hits no longer drop to zero on fork. Parent-only tools (agent / todo / ask_user_question / Plan Mode / MCP…) are registered in sub-agents as explicit error stubs with alternative guidance, fixing silent no-op calls caused by tool-set differences (e.g. bash aliased away). A budget guard rejects over-window requests: when the inherited prefix plus instructions exceed 90% of the window, messages are truncated at whole-turn boundaries (never splitting tool-call pairs); requests still over budget are refused outright (measured 1.44M over the limit used to end in HTTP 400). Fork instructions now state tool availability, and cache hit/miss token counts are persisted to JSONL so the savings are measurable
- **Web rate-limit backoff model (ThrottleStore)**: web_fetch/web_search share session-level per-host throttle state — exponential backoff on consecutive failures (from 2s, capped at 60s), Retry-After scheduling, a bounded 10-minute cooldown for 403s without Retry-After, and a minimum request gap after a failure on the same host. Requests inside a backoff window short-circuit without hitting the network. Error messages carry "N consecutive failures / earliest retry time" and distinguish rate-limiting from blocking, so the model no longer has to guess when to retry (it used to switch User-Agent and hammer the endpoint, amplifying the throttling). Retries get an independent timeout budget, and fork/cold sub-agents inherit the parent's throttle state instead of hammering a freshly limited host
- **Immediate background-task completion notices**: after every tool step the loop checks for finished background tasks and injects a completion notice (cursor shared with PrepareRun, naturally deduplicated) — the model no longer spins up its own sleep watcher. Todo snapshots are injected only when the status actually changes (repeated snapshots measured ~26% of all cache-miss tokens), with periodic visibility still covered by the reminder; the reminder text is now conditional (no update needed just to acknowledge it), and rate-limit-class errors no longer escalate to fatal
- **System events persisted with failure statistics**: model_error / tool_timeout / turn_duration / compaction / background_task notifications are persisted as structured `type=event` lines (bounded buffer with overflow re-recording, failed writes re-enqueued); the dead message_count field is backfilled; tool_errors / model_errors aggregate into the session stats. Resume/replay reads message lines only, so events never crowd out the conversation; TUI, runner and ACP report identical statistics

### Fixed
- **Duplicate /v1 in DeepSeek base_url returned 404**: the documented https://api.deepseek.com/v1 form or an OpenAI-compatible gateway address such as http://localhost:11434/v1 produced /v1/v1/chat/completions. A redundant trailing /v1 is now stripped at construction time (custom prefixes like /proxy/v1 are untouched) and setup hints differ per provider
- **A failed edit no longer rewrites the file**: when every hunk fails, normalize-and-write is skipped (it used to strip trailing whitespace and collapse blank lines, silently corrupting content such as Markdown double-space hard breaks on a failed edit). Output-sanitization character mismatches (e.g. circled digits ① vs "1") now get NFKC folding plus an explicit "consider write" diagnosis instead of infinite retries against seemingly identical text (root cause of a three-straight-failures session in Aug 2026); failure guidance moved to the top, hints deduplicated per file
- **skill install no longer copies .git**: when installing a repo whose SKILL.md sits at the root, copyDir used to copy the clone's .git directory into the install dir, and the supporting-files scan then listed .git internals in the Supporting files set. Both now skip dot-prefixed entries (directories via fs.SkipDir), each guarded by regression tests
