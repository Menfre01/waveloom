...
- ✅ `$ARGUMENTS` 无痛追加（body 中无 `$ARGUMENTS` 且 args 非空时自动追加 `ARGUMENTS: <value>`）
...
6. ── $ARGUMENTS 追加 ──
   若 body 中不含 $ARGUMENTS（替换后）且 args 非空：
     body += "\n\nARGUMENTS: " + args
...
25. **TestLoad_ArgsAutoAppend** — body 无 $ARGUMENTS 时自动追加 ARGUMENTS 行
26. **TestLoad_EmptyArgs** — args 为空时不追加 ARGUMENTS 行
27. **TestLoad_DynamicInjection** — `` !`command` `` 正确执行并替换
28. **TestLoad_MultilineInjection** — ` ```! ` 块正确执行
29. **TestLoad_DynamicInjectionTimeout** — 命令超时（30s）→ 返回超时占位符
30. **TestLoad_DynamicInjectionError** — 命令退出码非 0 → 输出包含 stderr
31. **TestLoad_DynamicInjectionNotAtLineStart** — 非行首 `!` 不触发动态注入
32. **TestLoad_ShellArgsQuoting** — 双引号参数正确解析
33. **TestLoad_EscapedDollar** — `\$` 转义正确保留为 `$`
34. **TestLoad_SkillNotFound** — 加载不存在的 skill → error
35. **TestLoad_SupportingFiles** — 附属文件清单正确追加到 body 末尾
36. **TestLoad_NoSupportingFiles** — 无附属文件时不追加清单
37. **TestLoad_FlatCommandFile** — .claude/commands/*.md 正确加载（无附属文件）
38. **TestFormatSkillListing** — 列表格式正确，包含 skill 名和描述，合并 when_to_use
39. **TestFormatSkillListing_Empty** — 无 skill 时返回空字符串
40. **TestFormatSkillListing_ExcludesDisabledModelInvocation** — disable-model-invocation 的 skill 不出现
41. **TestFormatSkillListing_TruncatesDescription** — description + when_to_use 超过 1536 字符时截断
42. **TestReload** — Reload 重新扫描并反映文件变更

### SkillTool 测试（`pkg/tool/skill_tool_test.go`）

43. **TestSkillTool_Execute** — 调用存在的 skill → 返回渲染后 body
44. **TestSkillTool_NotFound** — 调用不存在的 skill → Recoverable error
