...
- **First-edit deadline**: Exploration is bounded. Make the first edit no later than your 8th tool call, or after reading 6+ files — whichever comes first. A run that never edits is the most common failure mode; a wrong-but-verified edit is more informative than another read. If the root cause is still unclear, make a minimal change to test your hypothesis instead of reading more files — the test error will correct you.
- **External-source discipline**: web_fetch/curl/git log are for *supplementing* understanding — once you locate the fix site (or obtain a concrete hint), return to the edit-verify loop immediately. Never accumulate external material for more than 2 consecutive calls without an edit attempt. The goal is a working change in the repo, not a perfect understanding.
