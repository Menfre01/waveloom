...
> [!IMPORTANT]
> **安全透明**：Agent 写文件和执行命令前必须经过你确认，不会静默操作。**需要 API Key**：前往 [DeepSeek](https://platform.deepseek.com/api_keys) 获取，然后运行 `wvl setup` 完成配置。
...
```sh
wvl                      # 交互式 TUI 模式
wvl "解释 pkg/llm/client.go 的设计"  # 单次执行
wvl ls                   # 列出最近会话
...
其他平台替换为对应文件名即可。从源码构建：`cd waveloom && git pull && make install`。

> 关注 [Releases](https://github.com/Menfre01/waveloom/releases) 获取最新版本。
...
| 快速引用文件 | 输入 `@` 弹出文件选择器，模糊过滤 + `Tab` 进入子目录 |
| 恢复会话 | `wvl --continue` 恢复最近会话，`wvl --resume <id>` 恢复指定会话 |
| 查看日志 | `wvl --verbose` 启动，日志在 `.waveloom/wvl.log`，另一个终端 `tail -f` 实时查看 |
