...
DeepSeek 前缀缓存对比 `messages[0]` 起点找最长公共前缀，缓存命中价格仅为未命中的 **1/50 ~ 1/120**。Waveloom 通过固定 System Prompt 起点、跨轮累积消息历史、四级水位线压缩（Snip → Prune → Summarize → 硬截断），确保压缩后的字节永不变化，缓存命中率稳定在 **95-99%**。详见 [`docs/prefix-cache.md`](./docs/prefix-cache.md)。
